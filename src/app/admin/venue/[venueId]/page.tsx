...
export default async function AdminVenuePage({
  ...
  const reviewable = (item: { id: string }) => {
    const s = latestThisWeek.get(item.id) ?? latestEver.get(item.id);
    return s?.review === "pending" && s.progress === "done";
  };

  /**
   * The screen, house by house.
   *
   * Two people review, one per half, so every count on this page is one half's
  ...
  return (
    <main>
      ...
      {/* This week at a glance: see it, approve it, or send it back.
          ...
          One grid per house, front of house first. Approve-all is per house
          for the same reason the grade is: one person walked the dining room
          and another the kitchen, and a single button would have signed off
          work in the name of somebody who never saw it. */}
      {houses.map((group) => (
        <section key={group.house} className="mb-8">
          <div className="mb-3 flex items-center justify-between gap-3">
            <h2 className="label">
              {houseName(group.house)} · {group.awaitingReview} awaiting review
              {group.scored ? "" : " · practice"}
            </h2>
            {group.awaitingReview > 0 ? (
              <form action={approveAllForVenue}>
                <input type="hidden" name="venueId" value={venue.id} />
                <input type="hidden" name="weekStart" value={weekStart} />
                <input type="hidden" name="house" value={group.house} />
          ...
          <ul className="stagger grid grid-cols-1 items-stretch gap-3 sm:grid-cols-2 xl:grid-cols-5">
            ...
            {emptySlots(group.active.length, WEEKLY_ITEM_TARGET).map((slot) => (
              <AddItemSlot
                key={`slot-${group.house}-${slot}`}
                venueId={venue.id}
                house={group.house}
                index={slot}
              />
      ...
      <details className="mt-8">
        ...
        <section className="mt-8">
          ...
          {items.length === 0 ? (
            ...
            houses.map((group) => (
              <div key={group.house} className="mb-6">
                <h3 className="label mb-2">{houseName(group.house)}</h3>
                {/* Numbered within its own house, because the positions are.
                Numbered across the venue the kitchen would have run 11 to 20
                and the up arrow at the top of it would have looked broken —
                it moves within the house, and there is nothing above. */}
                <ul className="space-y-3">
                  {group.all.map((item, index) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <span className="label w-5 shrink-0">
                            {index + 1}
                          </span>
                          <RenameItemForm
                            itemId={item.id}
                        ...
                        <div className="mt-3 flex flex-wrap items-center gap-2">
                          ...
                          <form action={moveItem}>
                            ...
                            <button
                              type="submit"
                              className="btn-ghost"
                              disabled={index === group.all.length - 1}
