...
/** Everything under /close that is a screen rather than a checklist. */
...
export function EditThisList() {
  ...
  // Exactly /close/<slug>. Not /close itself, not the reports, and not the
  // editor, which would be a link to the page you are standing on.
  const parts = pathname.split("/").filter(Boolean);
  const onAList =
    parts.length === 2 && parts[0] === "close" && !NOT_A_LIST.has(parts[1]);
